...
config.plugins.iptvplayer.ytDefaultformat = ConfigSelection(default='720', choices=[('0', _('the worst')),
 ('144', '144p'),
 ('240', '240p'),
 ('360', '360p'),
 ('720', '720p'),
...
config.plugins.iptvplayer.ytSortBy = ConfigSelection(default='', choices=[('', _('Relevance')),
 ('video_date_uploaded', _('Upload date')),
 ('video_view_count', _('View count')),
 ('video_avg_rating', _('Rating'))])
    ...
    def getDirectLinks(self, url, formats = 'flv, mp4', dash = True, dashSepareteList = False, allowVP9 = None, allowAgeGate = None):
        ...
        if len(dashAudioLists):
            for item in dashVideoLists:
                item = dict(item)
                item['url'] = decorateUrl('merge://audio_url|video_url', {'audio_url': dashAudioLists[0]['url'],
                 'video_url': item['url']})
    ...
    def getSearchResult(self, pattern, searchType, page, nextPageCategory, sortBy = '', url = ''):
        ...
        try:
            ...
            if nP:
                ...
                urlNextPage = self.updateQueryUrl(url, {'pbj': '1',
                 'ctoken': ctoken,
                 'continuation': ctoken,
                 'itct': itct})
                params = {'type': 'more',
                 'category': 'search_next_page',
                 'title': label,
                 'page': str(int(page) + 1),
                 'url': urlNextPage}
                printDBG(str(params))
