	...
	def getSearchResult(self, pattern, searchType, page, nextPageCategory, sortBy=''):
		...
		def _shortdesc(url):
			...
			if url != '':
				...
				if sts:
					if data:
						tempdesc = self.cm.ph.getDataBeetwenMarkers(data, '"description":{"runs":[{"text":"', '"', False)[1]
						if len(tempdesc) > 0:
							tempdesc_a = str(tempdesc).strip().split('\\n')
							if len(tempdesc_a) > 0:
								for item in tempdesc_a:
									if (len(item) > 0) and (item.replace(' ','').replace('—','').replace('\\r','') != '') and (not item.startswith('http')):
										if tempdesc_new == '':
											tempdesc_new = item
										else:
											tempdesc_new = tempdesc_new + '   -|   ' + item
								if tempdesc_new.strip() != '':
									shortdesc = re.sub(r'^(.{580}).*$', '\g<1>...', tempdesc_new)
			return shortdesc
		
		currList = []
		try:
			url = 'https://www.youtube.com/results?search_query=%s&filters=%s&search_sort=%s&page=%s' % (pattern, searchType, sortBy, page)
			sts, data =  self.cm.getPage(url, self.http_params)
				...
				if data:
					...
					if len(data2) > 0:
						for item in data2:
							...
							currList.append(params)
						if len(currList) > 0:
							item_next = {'name': 'history', 'type': 'category', 'category': nextPageCategory, 'pattern':pattern, 'search_type':searchType, 'title': _("Next page"), 'page': str(int(page) + 1)}
							currList.append(item_next)
					else:
						if page > 1:
							MainSessionWrapper().open(MessageBox, _('No more videos!\n\n Go back to the previous page!'), type = MessageBox.TYPE_INFO, timeout = 5)
		except Exception:
			printExc()
			return []
		return currList
