# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/libs/youtube_dl/extractor/youtube.py
...
class CYTSignAlgoExtractor:
    ...
    def _findMainFunctionName(self):
        ...
        patterns = ['\\b[cs]\\s*&&\\s*[adf]\\.set\\([^,]+\\s*,\\s*encodeURIComponent\\s*\\(\\s*(?P<sig>[a-zA-Z0-9$]+)\\(',
         ...
         '(?P<sig>[a-zA-Z0-9$]+)\\s*=\\s*function\\(\\s*a\\s*\\)\\s*{\\s*a\\s*=\\s*a\\.split\\(\\s*""\\s*\\).*a\\.join\\(\\s*""\\s*\\)',
...
class YoutubeIE(object):
    ...
    _LANG_URL = 'https://www.youtube.com/?hl=en&persist_hl=1&gl=US&persist_gl=1&opt_out_ackd=1'
    _LOGIN_URL = 'https://accounts.google.com/ServiceLogin'
    _AGE_URL = 'http://www.youtube.com/verify_age?next_url=/&gl=US&hl=en'
    _NEXT_URL_RE = '[\\?&]next_url=([^&]+)'
    _NETRC_MACHINE = 'youtube'
    _available_formats = ['38',
     '37',
     '46',
     '22',
     '45',
     ...
     '171',
     '139']
    _available_formats_prefer_free = ['38',
     '46',
     '37',
     '45',
     '22',
    ...
    def __init__(self, params = {}):
        ...
        self.cm.HOST = 'Mozilla/5.0 (X11; Linux x86_64; rv:59.0) Gecko/20100101 Firefox/59.0'
        ...
        if not sts:
            ...
            try:
                ...
                if caption_url:
                    ...
                    list_params = urllib.urlencode({'type': 'list',
                     'tlangs': 1,
                     'asrs': 1})
                    ...
                    for lang_node in caption_list.findall('target'):
                        ...
                        for ext in self._SUBTITLE_FORMATS:
                            params = urllib.urlencode({'lang': original_lang,
                             ...
                             'ts': timestamp,
                             'kind': caption_kind})
                            sub_formats.append({'url': caption_url + '&' + params,
                             'ext': ext})
                ...
                for lang in caption_translation_languages.split(','):
                    ...
                    if not sub_lang:
                        continue
                    caption_qs.update({'tlang': [sub_lang],
                     'fmt': ['vtt']})
                    sub_url = urlunparse(parsed_caption_url._replace(query=urllib.urlencode(caption_qs, True)))
                    sub_tracks.append({'title': lang_qs['n'][0].encode('utf-8'),
                     'url': sub_url,
    ...
    def _real_extract(self, url, allowVP9 = False, allowAgeGate = False):
        ...
        if 'yt-video-id' == video_id:
            ...
            videoInfoparams = {'cookiefile': COOKIE_FILE,
             'use_cookie': True,
             'load_cookie': False,
             'save_cookie': True}
        else:
        ...
        if allowAgeGate and re.search('"LOGIN_REQUIRED"', video_webpage) is not None:
            age_gate = True
        ...
        try:
            mobj = re.search(';ytplayer.config = ({.*?});', video_webpage)
            if not mobj:
                raise ValueError('Could not find vevo ID')
            ytplayer_config = json_loads(mobj.group(1))
            args = ytplayer_config['args']
            if 'url_encoded_fmt_stream_map' not in args:
                raise ValueError('No stream_map present')
            re_signature = re.compile('[&,]s=')
        ...
        if not video_url_list:
            ...
            if self.cm.isValidUrl(dashmpd):
                sign = ph.search(dashmpd, '/s/([a-fA-F0-9\\.]+)')[0]
                if sign:
    ...
    def _extract_from_m3u8(self, manifest_url, video_id):
        ...
        for format_url in formats_urls:
            itag = self._search_regex('itag/(\\d+?)/', format_url, 'itag')
    ...
    def _get_video_url_list(self, url_map, allowVP9 = False):
        format_list = list(self._available_formats_prefer_free)
        if allowVP9:
            format_list.extend(['313', '271'])
        existing_formats = [ x for x in format_list if x in url_map ]
        return [ (f, url_map[f]) for f in existing_formats ]