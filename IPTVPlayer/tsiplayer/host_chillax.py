...
class TSIPHost(TSCBaseHostClass):
    def __init__(self):
        ...
        self.MAIN_URL = 'https://ww4.9movies.yt'
    ...
    def get_links(self,cItem):
        ...
        if sts:
            Tab_els = re.findall('href="(/watch/.*?)"', data, re.S)
            if Tab_els:
                URL = self.MAIN_URL+Tab_els[0]
                sts, data = self.getPage(URL)
                if sts:
            ...
            if Tab_els:
                ...
                if Tab_els:
                    ...
                    if '?id=' in url:
                        ...
                        if '&' in id: id,x2 = id.split('&',1)
                        if '&sub=' in url: x1,sub_url = url.split('&sub=',1)
                        else: sub_url =''
                        
                        if sub_url!='':
                            url = 'https://m4ufree.yt/playlist/'+id+'/1614829290001'
                            url = sub_url+'|Eng|'+url
                            urlTab.append((url,'6'))
                        else:
                            url = 'https://m4ufree.yt/playlist/'+id+'/1614829290001'
