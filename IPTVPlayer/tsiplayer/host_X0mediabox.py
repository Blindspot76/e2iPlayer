...
class TSIPHost(TSCBaseHostClass,CaptchaHelper):
    ...
    def showitms_03(self,cItem):
        ...
        if sts:
            ...
            for elm in data.get('cast',[]):
                ...
                name_ = elm1['name']
                url_ = 'https://api.themoviedb.org/3/person/'+str(id_mo_)+'/external_ids?api_key='+base64.b64decode(self.api_key)
                img_=''
                if i<15:
                    url_image = 'https://api.themoviedb.org/3/person/'+str(id_mo_)+'/images?api_key='+base64.b64decode(self.api_key)
            ...
            for elm1 in streams:
                ...
                if (stream not in stream_lst):
                    ...
                    if type_ in [9004]:
                        ...
                        Type_ = 'Momomesh'
                        Desc  = 'Multi Hosts'
                        Image = 'https://momomesh.tv/wp-content/uploads/2020/07/momomesh.logo_.hd_.png'
                    elif type_ in [9000]:
                        Name  = 'GDDB [Multi]'
                        Url   = stream
                        Type_ = 'GDDB'
                        Desc  = 'Multi Hosts'
                        Image = 'https://cdn.domestika.org/c_fill,dpr_2.0,h_96,t_base_params.format_jpg,w_96/v1311703747/avatars/000/054/289/54289-original.jpg'	                        
                    elif type_ in [9001]:
                        Name  = 'OpenloadMovies [Multi]'
                        Url   = stream
                        Type_ = 'openloadmovies'
                        Desc  = 'Multi Hosts'
                        Image = 'https://openloadmovies.cam/wp-content/uploads/2020/02/openload-movie-desktop.png'	                                        
                        ...
                        #48:pl ,43:kisscartoon,49:tvz  | Cloudflare
                        Name  = '|'+str(type_)+'| '+name
                        Url   = stream
                        Type_ = 'Non'#'na'
                        Desc  = stream                 
                        ...
                        if os.path.exists('/usr/lib/enigma2/python/Plugins/tsiplayer'):
                            Type_ = 'na'
                            Name  = Name + ' <---------- NEW'
                            Image = 'https://i.ibb.co/Q8ZRP0X/yaq9y3ab.png'
                        else:
                            Type_ = 'Non'#'na'       
                        Desc  = stream
                    if 	Type_ != 'Non':				
                        printDBG('elm'+str(elm1))
    ...
    def get_links(self,cItem):
        ...
        if Type_ == 'VIP':
            ...
            from tsiplayer.host_momomesh import TSIPHost
            host = TSIPHost()				
            urlTab.extend(host.get_links({'url': Url}))	
        elif Type_ == 'openloadmovies':
            from tsiplayer.host_openloadmovies import TSIPHost
            host = TSIPHost()				
            urlTab.extend(host.get_links({'url': Url}))	
        elif Type_ == 'GDDB':	
            urlTab = self.Get_GDDB_Servers(Url)	               
        elif Type_ == 'Vidlink':
