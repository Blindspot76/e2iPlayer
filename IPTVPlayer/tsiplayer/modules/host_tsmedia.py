...
def getinfo():
	version_f='/usr/lib/enigma2/python/Plugins/Extensions/TSmedia/version'
	with open(version_f) as f:
		lines = f.readlines()
	version = lines[0].split(':')[1].strip()		
	info_={}
	info_['name']='TSMedia'
	info_['version']='1.0 11/11/2019'+' | '+tscolor('\c0000????')+version+ ' (TSMedia)'
	info_['dev']='mfaraj57'
	info_['cat_id']='903'
	info_['desc']='TSMedia Enigma2 Addon'
	info_['icon']='file:///usr/lib/enigma2/python/Plugins/Extensions/TSmedia/interface/images/team.png'
	return info_
...
class TSIPHost(TSCBaseHostClass):
	def __init__(self):
		...
		paths = ['script.module.main/lib','script.module.urlresolver/lib','script.module.urlresolver/lib/urlresolver']	
		for path in paths:
	...
	def tsmedia_getlist(self,lst,cItem):
		...
		for elm in lst:  
			...
			if ('plugin.video.youtube' in url_) and  ('&videoid=' in url_):
				...
				if 'youtube' in url_:
					self.addVideo({'import':cItem['import'],'category' : 'video','good_for_fav':True,'hst':'none','title':titre,'url':url_,'desc':desc_,'icon':img_})				
				else:
					self.addVideo({'import':cItem['import'],'category' : 'video','good_for_fav':True,'hst':'direct','title':titre,'url':url_,'desc':desc_,'icon':img_})	
			elif (self.ts_up.checkHostSupport(url_) == 1) and config.plugins.iptvplayer.ts_resolver.value=='tsiplayer':
				URL=url_
				self.addVideo({'import':cItem['import'],'category' : 'video','good_for_fav':True,'hst':'none','title':titre.title(),'url':URL,'desc':desc_,'icon':img_,'py_file':cItem['py_file'],'section':cItem['section'],'plugin_id':cItem['plugin_id'],'argv2':str(elm),'gnr':'menu2',})					
			else:
				self.addDir({'import':cItem['import'],'category' :'host2','good_for_fav':True,'argv2':str(elm),'title':titre.title(),'desc':desc_,'icon':img_,'py_file':cItem['py_file'],'section':cItem['section'],'plugin_id':cItem['plugin_id'],'type_':type_,'mode':'20',})		
