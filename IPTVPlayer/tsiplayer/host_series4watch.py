...
def getinfo():
	...
	info_['icon']='https://tv.series4watch.online/themes/Series4Watch/img/logo.png'
...
class TSIPHost(TSCBaseHostClass):
	def __init__(self):
		...
		self.MAIN_URL = 'https://tv.series4watch.online'
	...
	def showmenu1(self,cItem):
		...
		if count==1:
			...
			if sts:
				data1=re.findall('dropdown select-menu">(.*?)</ul>', data, re.S)
			else:
				data1='' 
		if count==4:
			mode_='30' 
			...
			mode_='20'
		lst_data1 = re.findall('<div.*?data-tax="(.*?)".*?data-cat="(.*?)".*?bold">(.*?)<',data1[count-1], re.S)	
	...
	def showitms(self,cItem):
		...
		if gnr=='items':
			...
			if sts:
				desc=''
				desc_data=re.findall('<p class="Scroll-list".*?">(.*?)</p>', data, re.S)
				if desc_data:
			...
			if sts:		 
				cat_data=re.findall('class="row.*?(.*?)class="pagination">', data, re.S)
				if cat_data:
					data=cat_data[0]
					cat_data=re.findall('data-src="(.*?)".*?href="(.*?)"(.*?)<h3>(.*?)<.*?>(.*?)<div', data, re.S)
					for (image,url1,inf,name_eng,desc) in cat_data:
						desc1=''
						lst_dat=re.findall('category">(.*?)<', inf, re.S)
						if lst_dat: desc1=desc1+'Category: '+ lst_dat[0] +'\\n'
						lst_dat=re.findall('rating">(.*?)</', inf, re.S)
						if lst_dat: desc1=desc1+'Rating: '+ ph.clean_html(lst_dat[0]) +'\\n'
						desc=desc1+desc
						self.addDir({'import':cItem['import'],'good_for_fav':True,'EPG':True,'hst':'tshost','category' : 'host2','url': url1,'title':name_eng,'desc':ph.clean_html(desc),'icon':image,'sub_mode':'items','mode':'30'})			
	...
	def getArticle(self, cItem):
		...
		if sts:
			lst_dat=re.findall('<div class="title">(.*?)class="section-head">', data, re.S)
			if lst_dat:
				lst_dat0=re.findall('class="row">(.*?)</div>', lst_dat[0], re.S)
