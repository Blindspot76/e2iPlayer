...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.resources.lib.comaddon import progress, VSlog, isMatrix, dialog
...
def SIGN(pth, version=4):
    ...
    rawtxt = '/v%s/%s?drms=dt1,dt2&device_id=%s&app=%s' % (version, pth, _DEVICE_ID, _APP)
    sig = hmac.new(
        _APP_SECRET.encode('ascii'), ('%s&t=%s' % (rawtxt,timestamp)).encode('ascii'), hashlib.sha1).hexdigest()
